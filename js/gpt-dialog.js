...
export function getSystemPrompt({ userName='친구', userAge=0, verbosity='default', elapsedTime=0, userTraits=[] }={}, intent='fact') {
  const voc = getKoreanVocativeParticle(userName); // "라이언아" 또는 "민후야"
  const nameWithVocative = `${userName}${voc}`;     // "라이언아" 또는 "민후야"
  
  const subjectParticle = getKoreanSubjectParticle(userName); // "이" 또는 "가"
  const nameWithSubjectParticle = `${userName}${subjectParticle}`; // "라이언이" 또는 "민후가"
  ...
  if (verbosity === 'short') {
    ...
    prompt += `\n[기본 답변 길이] 사용자의 말이 짧을 때는(예: 40글자 미만) 로지의 답변도 1-2문장, 최대 20글자 이내로 매우 간결하게 유지하세요. 사용자의 말이 길어지면(예: 150토큰 이상) 로지의 답변도 조금 더 길어져도 괜찮지만, 항상 사용자의 발화량보다 약간 적게 유지하는 것이 좋습니다.`;
  }
